...
import CompanyUserManagement from '@/components/manager/CompanyUserManagement';
...
export default function CompanyTasksPage({ params }) {
  ...
  }, [user, userRole, loading, router, companyId]);

  const fetchCompanyAndTasks = async () => {
    setIsLoadingData(true);
    try {
      // Fetch company details
      const companiesResult = await getUserCompanies(user.uid);
      if (companiesResult.success) {
        const foundCompany = companiesResult.data.find(
      ...
      const [myTasksRes, allTasksRes] = await Promise.all([
        getManagerTasks(user.uid, companyId),
  ...
  if (!user || userRole !== 'manager' || !company) return null;
