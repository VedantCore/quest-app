      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">
            {isStaff ? 'My Account' : `Welcome, ${user.displayName || 'Explorer'}`}
          </h1>
        ...
        {!isStaff && activeTab === 'tasks' && stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
              <p className="text-xs text-gray-500 font-bold uppercase">Active Quests</p>
              <p className="text-2xl font-bold text-[#13B5A0]">{stats.activeTasks}</p>
            </div>
            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
              ...
              <p className="text-2xl font-bold text-orange-500">{stats.overallProgress}%</p>
            </div>
          </div>
        )}
        ...
        {!isStaff && (
          <div className="bg-white p-1 rounded-xl inline-flex border border-gray-200 shadow-sm mb-6">
            <button
              onClick={() => { setActiveTab('tasks'); window.history.pushState(null, '', '?tab=tasks'); }}
              className={`px-6 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === 'tasks' ? 'bg-gray-900 text-white' : 'text-gray-500 hover:text-gray-900'}`}
            >
              Quests Board
            </button>
            <button
              onClick={() => { setActiveTab('profile'); window.history.pushState(null, '', '?tab=profile'); }}
              className={`px-6 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === 'profile' ? 'bg-gray-900 text-white' : 'text-gray-500 hover:text-gray-900'}`}
            >
              Profile
            </button>
          </div>
