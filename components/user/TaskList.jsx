  ...
  if (companies.length === 0 && filteredTasks.length === 0) {
     return (
        <div className="rounded-xl border border-gray-200 bg-white p-10 text-center">
          ...
          <p className="text-gray-600">You are not joined to any companies yet.</p>
        </div>
      );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      {companies.map(company => {
        ...
        return (
            <div key={company.company_id} className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm transition-shadow hover:shadow-md">
                <button 
                  onClick={() => toggleCompany(company.company_id)}
                  className={`w-full flex items-center justify-between p-4 text-left transition-colors ${
                      isExpanded ? 'bg-white border-b border-gray-100' : 'bg-gray-50/50 hover:bg-gray-50'
                  }`}
                >
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500 font-medium">
                                {companyTasks.length} {companyTasks.length === 1 ? 'task' : 'tasks'} available
                            </p>
                        </div>
                    </div>

                    <div className={`p-2 rounded-full text-gray-400 transition-transform duration-200 ${isExpanded ? 'rotate-180 bg-gray-100 text-gray-600' : ''}`}>
                      <svg 
                        className="w-5 h-5" 
                        fill="none" 
                        viewBox="0 0 24 24" 
