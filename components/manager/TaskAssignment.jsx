...
export default function TaskAssignment() {
  ...
  if (loading)
    return <div className="p-4">{t('manager.assignment.loading')}</div>;
    ...
    <div className="bg-white shadow rounded-lg p-6 relative">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold text-gray-900">
          {t('manager.assignment.pendingSubmissions')}
        </h2>
        <button
          onClick={loadSubmissions}
          className="text-sm text-indigo-600 hover:text-indigo-800"
        >
          {t('manager.assignment.refresh')}
        </button>
      </div>

      {submissions.length === 0 ? (
        <div className="text-center py-8 text-gray-500">
          {t('manager.assignment.noSubmissions')}
        </div>
      ) : (
            ...
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  {t('manager.assignment.quest')} / {t('manager.task.step')}
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  {t('manager.assignment.user')}
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  {t('manager.assignment.submitted')}
                </th>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    ...
                    >
                      {processingId === submission.submission_id
                        ? t('manager.task.processing')
                        : t('manager.assignment.review')}
                    </button>
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6 m-4">
            <h3 className="text-lg font-bold text-gray-900 mb-4">
              {t('manager.assignment.reviewSubmission')}
            </h3>
